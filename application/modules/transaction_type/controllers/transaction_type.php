...
class Transaction_type extends MX_Controller {
...
// edit claim type
    ...
    function get_table() {
        ...
        $table = $this->transaction_type_model->get_table();
        return $table;
    }

    function get($order_by) {
        $this->load->model('transaction_type_model');
        $query = $this->transaction_type_model->get($order_by);
        return $query;
    }

    function get_with_limit($limit, $offset, $order_by) {
        $this->load->model('transaction_type_model');
        $query = $this->transaction_type_model->get_with_limit($limit, $offset, $order_by);
        return $query;
    }

    function get_where($id) {
        $this->load->model('transaction_type_model');
        $query = $this->transaction_type_model->get_where($id);
        return $query;
    }

    function get_where_custom($col, $value) {
        $this->load->model('transaction_type_model');
        $query = $this->transaction_type_model->get_where_custom($col, $value);
        return $query;
    }

    function _insert($data) {
        $this->load->model('transaction_type_model');
        $this->transaction_type_model->_insert($data);
    }

    function _update($id, $data) {
        $this->load->model('transaction_type_model');
        $this->transaction_type_model->_update($id, $data);
    }

    function _delete($id) {
        $this->load->model('transaction_type_model');
        $this->transaction_type_model->_delete($id);
    }

    function count_where($column, $value) {
        $this->load->model('transaction_type_model');
        $count = $this->transaction_type_model->count_where($column, $value);
        return $count;
    }

    function get_max() {
        $this->load->model('transaction_type_model');
        $max_id = $this->transaction_type_model->get_max();
        return $max_id;
    }

    function _custom_query($mysql_query) {
        $this->load->model('transaction_type_model');
        $query = $this->transaction_type_model->_custom_query($mysql_query);
        return $query;
    }
