...
// default function
    function index() {
        ...
        if (!$this->ion_auth->logged_in()) {
            ...
            $sql = "select * from domain where id_domain=" . $this->ion_auth->get_id_domain();
            $sql.=" order by name";
...
// edit insurer
    ...
    function get_table() {
        ...
        $table = $this->domain_model->get_table();
        return $table;
    }

    function get($order_by) {
        $this->load->model('domain_model');
        $query = $this->domain_model->get($order_by);
        return $query;
    }

    function get_with_limit($limit, $offset, $order_by) {
        $this->load->model('domain_model');
        $query = $this->domain_model->get_with_limit($limit, $offset, $order_by);
        return $query;
    }

    function get_where($id) {
        $this->load->model('domain_model');
        $query = $this->domain_model->get_where($id);
        return $query;
    }

    function get_where_custom($col, $value) {
        $this->load->model('domain_model');
        $query = $this->domain_model->get_where_custom($col, $value);
        return $query;
    }

    function _insert($data) {
        $this->load->model('domain_model');
        $this->domain_model->_insert($data);
    }

    function _update($id, $data) {
        $this->load->model('domain_model');
        $this->domain_model->_update($id, $data);
    }

    function _delete($id) {
        $this->load->model('domain_model');
        $this->domain_model->_delete($id);
    }

    function count_where($column, $value) {
        $this->load->model('domain_model');
        $count = $this->domain_model->count_where($column, $value);
        return $count;
    }

    function get_max() {
        $this->load->model('domain_model');
        $max_id = $this->domain_model->get_max();
        return $max_id;
    }

    function _custom_query($mysql_query) {
        $this->load->model('domain_model');
        $query = $this->domain_model->_custom_query($mysql_query);
        return $query;
    }
