                ...
                <tbody>
                    <?php foreach ($insurances as $insurance): ?>          
                        <tr>
                            ...
                            <td><?php $property=($insurance->type==1) ? Modules::run("property/get_where",$insurance->id_property)->row()->number:Modules::run("property/get_where",$insurance->id_property)->row()->description ;  echo  anchor(site_url('property/view/' . $insurance->id_property), $property, 'View Property'); ?></td>
                            ...
                            <?php echo anchor("claim/register/" . $insurance->id_insurance, '<i class="fa fa-warning"></i>', 'title="Register Claim"');}?>
                                <?php if(((round(($insurance->expired_on-time()))/60/60/24/30)<1 || $insurance->expired_on <= time()) && !$insurance->closed){ echo anchor("insurance/renew/" . $insurance->id_insurance, '<i class="fa fa-credit-card"></i>', 'title="Renew"'); }?>
                                </td>
                        </tr>
